...
program
  ...
  .description('Upload a table document to a Confluence page')
  .version('1.0.0')
...
async function processMermaidDiagrams(data: any, pageId: string): Promise<Map<string, string>> {
  ...
  for (let index = 0; index < data.table.length; index++) {
    ...
    if (row.diagram) {
      console.log(`  Processing Mermaid diagram for row ${index + 1}: ${row.use_case.substring(0, 50)}...`)

      const filename = `diagram-${index}.png`

      // Escape mermaid code for shell
      const escapedMermaid = row.diagram
        .replace(/\\/g, '\\\\')
        .replace(/"/g, '\\"')
        .replace(/\$/g, '\\$')
        .replace(/`/g, '\\`')
      ...
      try {
        ...
        if (result.success) {
          attachmentMap.set(index.toString(), filename)
          console.log(`  ✓ Uploaded: ${result.attachment.title}`)
        } else {
          throw new Error(result.error)
        }
      } catch (error: any) {
        throw new Error(`Failed to upload Mermaid diagram for row ${index + 1}: ${error.message}`)
      }
    }
  }
...
async function updateConfluencePage(
  ...
  const response = await axios.put(
    `https://emarsys.jira.com/wiki/api/v2/pages/${pageId}`,
    {
      ...
      version: {
        number: version + 1,
        message: 'Updated via table document script',
      },
    },
    createSettings()
  )
...
async function main() {
  try {
    ...
    console.log('Converting table document to Confluence storage format...')
